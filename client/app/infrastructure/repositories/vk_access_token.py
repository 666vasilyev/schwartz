"""
Токены VK только из Postgres (таблица vk_access_tokens, синхронна с сервером).
При ошибке подключения к БД — токен недоступен.
"""
...
async def count_active_vk_tokens(session: AsyncSession) -> int:
    q = (
        select(func.count())
        .select_from(VkAccessToken)
        .where(VkAccessToken.is_active.is_(True))
    )
