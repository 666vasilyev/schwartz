"""
HTML → читаемый текст для полей из внешних источников (RSS summary/content).

Многие RSS-ленты кладут в description/content валидный HTML (<p>, <a>, <img>,
списки и т.д.) вместо чистого текста. Реализовано на стандартной библиотеке
(html.parser) — без новых зависимостей для этого сервиса (см. requirements.txt:
клиент нарочно лёгкий, без БД/LLM/torch, отдельный деплой от server).
"""
...
def strip_html(raw: str | None) -> str | None:
    ...
    try:
        ...
        # Битая/нестандартная разметка — не роняем сбор ленты, отдаём исходный текст.
        return text
