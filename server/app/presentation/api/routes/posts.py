...
async def export_posts(
    ...
    source_ids: list[int] | None = Query(None, ge=1, description="Фильтр по источникам"),
    category_names: list[str] | None = Query(None, description="Фильтр по категориям источника"),
...
async def list_posts(
    ...
    source_ids: list[int] | None = Query(None, description="ID источника"),
    category_names: list[str] | None = Query(None, description="Фильтр по категориям источника (можно несколько)"),
    date_from: datetime | None = Query(None, description="Начало диапазона (published_at >=)"),
    date_to: datetime | None = Query(None, description="Конец диапазона (published_at <=)"),
    db: AsyncSession = Depends(get_session),
) -> PostListResponse:
