...
class Settings(BaseSettings):
    ...
    # Вес нового поста в EMA-центроиде кластера (0..1): c' = alpha*v + (1-alpha)*c.
    # Раньше центроид считался как running mean по ВСЕЙ истории кластера — при
    # тысячах постов вес нового поста стремится к 0, центроид "замерзает" и
    # вырождается в среднее по всему корпусу, которое из-за анизотропии
    # embedding-пространства оказывается неожиданно похоже почти на что угодно
    # ("хаб"-эффект: кластер начинает пожирать всё подряд). EMA держит центроид
    # актуальным для недавнего содержимого, а не всей истории с момента создания.
    cluster_centroid_ema_alpha: float = 0.2
    # Жёсткий потолок размера кластера: при достижении — кластер перестаёт быть
    # кандидатом для новых постов (принудительно создаётся новый кластер).
    # Страховка от мега-кластеров независимо от качества центроида.
