...
async def execute(db: AsyncSession, source_id: int) -> SourceRead:
    ...
    if row.status in (SourceStatus.DELETED, SourceStatus.BLOCKED):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Нельзя поставить на паузу источник со статусом {row.status}",
        )

    prev_status = row.status
    updated = await set_source_status(db, source_id, SourceStatus.PAUSED.value)
    ...
    await add_audit_log(
        ...
        changes={"status": SourceStatus.PAUSED.value},
    )
